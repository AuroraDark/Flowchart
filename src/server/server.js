...
app.post("/upload", (req, res) => {
    ...
    fs.readFile('../data/c.json', function(err, data) {
        if(err){
          ...
          let c_string = JSON.stringify(c_loadData);
        //   c_string = c_string.replace('[', '[\n');
        //   c_string = c_string.replace(/{/g, '\t{');
        //   c_string = c_string.replace(/},/g, '\n\t},');
        //   c_string = c_string.replace(/,/g, ',\n');
        //   c_string = c_string.replace(/{"id"/g, '{\n\t\t"id"');
        //   c_string = c_string.replace(/"id_a"/g, '\t\t"id_a"');
        //   c_string = c_string.replace(/"id_b"/g, '\t\t"id_b"');
        //   c_string = c_string.replace(/"display"/g, '\t\t"display"');
        //   c_string = c_string.replace('}]', '\n\t}\n]\n');
          fs.writeFile('../data/c.json', c_string, (err) => {
              if (err) {
