...
const ProcedureTable = () => {
  ...
  return (
    ...
    <TableContainer component={Paper}>
      <Box
        style={{ overflowY: "scroll", height: "470px", scrollbarWidth: "thin" }}
        sx={{
          flexDirection: "row",
          p: 1,
          m: 1,
      ...
      >
        <Table>
          ...
          <TableBody>
            {b.map((data, key) => (
              <TableRow className="height-procedure" key={key} hover={true}>
                <TableCell>{data.name}</TableCell>
                <TableCell>{data.id}</TableCell>
                <TableCell></TableCell>
                <TableCell>
                  <Stack direction="row" spacing={0}>
                    <IconButton aria-label="edit" color="primary" onClick={handleClickOpen_EditDialog}>
    ...
    <AddProcedure handleClose_AddDialog={handleClose_AddDialog} addOpen={addOpen} />
    <EditProcedure handleClose_EditDialog={handleClose_EditDialog} editOpen={editOpen} />
    </>
  );
};
